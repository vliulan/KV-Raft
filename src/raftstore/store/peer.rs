...
pub static mut get_request_num: u64 = 0;    // wyy add
pub static mut last_get_request_num: u64 = 0;    // wyy add

pub static mut get_log_num: u64 = 0;    // wyy add
pub static mut last_get_log_num: u64 = 0;    // wyy add

pub static mut gap_sum: u64 = 0;    // wyy add
pub static mut last_gap_sum: u64 = 0;    // wyy add
...
type Key = Vec<u8>; // kv-raft wyy add

pub struct NoApply {
    pub index: u64,
    pub key: Key,  // kv-raft wyy add  
}
...
pub struct Peer {
    ...
    // wyy add
    pub index_cbs: Vec<IndexCb>,
    pub read_index_cbs: Vec<ReadIndexCb>,
    pub no_applies: Vec<NoApply>,   // kv-raft wyy add
...
impl Peer {
    ...
    fn new<T, C>(
        ...
        let raft_cfg = raft::Config {
            ...
            check_quorum: true,
            tag: tag.clone(),
            skip_bcast_commit: false,   // wyy change true to false
            ..Default::default()
        };
        ...
        let mut peer = Peer {
            ...
            raft_group,
            index_cbs: vec![],  // wyy add
            read_index_cbs: vec![],  // wyy add
            no_applies: vec![], // kv-raft wyy add
    ...
    // wyy add. 在step()和post_apply()中加入
    pub fn handle_wr(&mut self) {
        // wyy add. leader把写请求的cb及其index加到index_cbs队列
        // 每次leader处理完Raft消息时时，检测index_cbs队列，如果index <= leader的apply index，移出index_cbs队列
        // 否则如果index <= 某个follower的apply index，正确响应cb，并移出index_cbs队列
        if self.is_leader() {
            // 处理写请求，即index_cbs队列
            while self.index_cbs.len() > 0 {
                let index = self.index_cbs[0].index;
                if self.raft_group.raft.raft_log.committed >= index {   // commit return
                //if self.raft_group.raft.raft_log.applied >= index {   // apply return
                    let mut resp = RaftCmdResponse::new();
                    match self.index_cbs.pop() {
                        Some(index_cb) => {
                            index_cb.cb.invoke_with_response(resp);
                        }
                        None => break,
                    }
                } else {
            ...
            // 处理no_applies队列的写请求
            while self.no_applies.len() > 0 {
                let index = self.no_applies[0].index;
                if self.raft_group.raft.raft_log.applied >= index {
                    self.no_applies.pop();
                } else {
                    break;
                }
            ...
            // read index读
            // 处理读请求，即read_index_cbs队列
            while self.read_index_cbs.len() > 0 {
                let index = self.read_index_cbs[0].index;
                //println!("step read, read_index: {:?}, leader_applied: {:?}", index, self.raft_group.raft.raft_log.applied); // wyy add
                if self.raft_group.raft.raft_log.applied >= index {
                    // leader能直接响应读请求
                    let mut resp = RaftCmdResponse::new();
                    match self.read_index_cbs.pop() {
                        Some(read_index_cb) => {
                            read_index_cb.cb.invoke_read(self.handle_read(read_index_cb.req));
                        }
                        None => break,
                    }
                } else {break;}
	...
	//self.poll_apply(); // wyy add. 更新apply状态

	self.handle_wr();	// wyy add
        // wyy add. leader把写请求的cb及其index加到index_cbs队列
        // 每次leader处理完Raft消息时时，检测index_cbs队列，如果index <= leader的apply index，移出index_cbs队列
        // 否则如果index <= 某个follower的apply index，正确响应cb，并移出index_cbs队列
        /*if self.is_leader() {
    ...
    fn apply_reads(&mut self, ready: &Ready) {
        ...
        if self.ready_to_handle_read() {
            for state in &ready.read_states {
                // wyy add. Ensure apply index >= read index
                if self.raft_group.raft.raft_log.applied < state.index {
                    break;
                }
	...
	// wyy add. applied改变时leader给自己发raft消息
        /*if(self.raft_group.raft.id == self.raft_group.raft.leader_id) {
            let mut to_send = eraftpb::Message::new();
            to_send.set_to(self.raft_group.raft.leader_id);
            to_send.set_msg_type(eraftpb::MessageType::MsgHeartbeatResponse);
            //to_send.set_context(m.take_context());
            //self.raft.last_applied = self.raft.raft_log.applied;
            //to_send.set_commit(self.raft.raft_log.applied);
            self.raft_group.raft.send(to_send);
        }*/
	...
	//self.handle_wr();       // wyy add
        ...
        let mut flag = 0;  // wyy add
        let mut key: Key = vec![];  // kv-raft wyy add 
        let res = match self.get_handle_policy(&req) {
            ...
            Ok(RequestPolicy::ReadIndex) | Ok(RequestPolicy::ReadLocal) => {
                // wyy add.
                let gap = self.raft_group.raft.raft_log.committed - self.raft_group.raft.raft_log.applied;
                unsafe {
                    gap_sum += (gap as u64);
                    get_request_num += 1;
                    let i:u64 = 10000;
                    if get_request_num >= last_get_request_num + i {
                        println!("propose get_request_num: {:?}, get_log_num: {:?}, gap_sum: {:?}, gap_avg: {:?}", get_request_num, get_log_num, gap_sum, (gap_sum-last_gap_sum) as f64 / i as f64);
                        last_get_request_num = get_request_num;
                        last_gap_sum = gap_sum;
                    } 
                }

                // direct read
		if req.requests.len() > 0 {
                    key = req.requests[0].get_get().key.clone();    // kv-raft wyy add
                    //println!("key.len: {:?}", req.requests[0].get_get().key.len());
                } 
                for no_apply in self.no_applies.iter() {
                    if no_apply.index > self.raft_group.raft.raft_log.committed {   // 不能读没commit的数据
                        break;
                    } else{
                        //println!("requests.len: {:?}, key: {:?}, no_apply.key: {:?}", req.requests.len(), key, no_apply.key);
                        if no_apply.key == key {
                            unsafe {
                                get_log_num += (req.requests.len() as u64);
                            }
                            let mut resp = RaftCmdResponse::new();
                ...
                // read index读
                let read_index_cb = ReadIndexCb {
                    req: req,
                    index: self.raft_group.raft.raft_log.committed,
                    cb: cb,
                };
                self.read_index_cbs.push(read_index_cb);
        
                // wyy add. 节点propose把读请求加到队列后，立即处理读请求，即read_index_cbs队列
                // 不能加到队列前处理，不然很慢
                while self.read_index_cbs.len() > 0 {
                    let index = self.read_index_cbs[0].index;
                    //println!("step read, read_index: {:?}, leader_applied: {:?}", index, self.raft_group.raft.raft_log.applied); // wyy add
            ...
            //Ok(RequestPolicy::ProposeNormal) => self.propose_normal(req, metrics),    // wyy delete
            // wyy add
            Ok(RequestPolicy::ProposeNormal) => {
                flag = 1; 
                if req.requests.len() > 0 {
                    key = req.requests[0].get_put().key.clone();    // kv-raft wyy add
                }
                self.propose_normal(req, metrics)
            }
        ...
        match res {
            ...
            Ok(idx) => {
                ...
                // 否则如果index <= 某个follower的apply index，正确响应cb，并移出index_cbs队列
                if flag == 1 {
                    let index_cb = IndexCb {
                        index: idx,
                        cb: cb,
                    };
                    self.index_cbs.push(index_cb);

                // kv-raft wyy add
                let no_apply = NoApply {
                    index: idx,
                    key: key, 
                };
                self.no_applies.push(no_apply);
