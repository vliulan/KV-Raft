...
use kvproto::raft_cmdpb::{CmdType, DeleteRangeRequest, DeleteRequest, PutRequest, RaftCmdRequest,
                          RaftCmdResponse, RaftRequestHeader, Request, Response, GetRequest}; // wyy add GetRequest
...
impl<S: RaftStoreRouter> Engine for RaftKv<S> {
    ...
    // wyy add
    fn async_snapshot_raft(&self, ctx: &Context, cb: Callback<Box<Snapshot>>, key: Vec<u8>) -> engine::Result<()> {
        fail_point!("raftkv_async_snapshot");
        let mut req = Request::new();
        req.set_cmd_type(CmdType::Snap);
	// wyy add
